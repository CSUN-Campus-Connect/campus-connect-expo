...
import {
  ...
  mockStudentProfiles,
  mockStudyGroups,
  seedSelectedSemesterId,
  seedSemesters,
} from './shared/mockData';
import { loadStateAsync, makeId, norm, saveStateAsync } from './shared/utils';

export type AssignmentWithCourse = import('./shared/constants').Assignment & {
  courseCode: string;
  courseId: string;
};
...
export function useAcademicsData() {
  ...
  const [semesters, setSemesters] = React.useState(seedSemesters);
  ...
  const [studyGroups, setStudyGroups] = React.useState(mockStudyGroups);
  ...
  React.useEffect(() => {
    let cancelled = false;
    (async () => {
      const loaded = await loadStateAsync(LS_KEY);
      if (cancelled || !loaded) {
  ...
  React.useEffect(() => {
    if (!hydrated) return;
    void saveStateAsync(LS_KEY, { semesters, selectedSemesterId });
  }, [semesters, selectedSemesterId, hydrated]);
  ...
  const upcomingAssignments = React.useMemo((): AssignmentWithCourse[] => {
    const all: AssignmentWithCourse[] = [];
    for (const c of selectedSemester?.courses ?? []) {
      for (const a of c.assignments ?? []) {
        all.push({
          ...a,
          courseCode: `${c.subject} ${c.number}`,
          courseId: c.id,
        });
      }
    }
  ...
  const upcomingExams = React.useMemo(() => {
    const all: Array<import('./shared/constants').ExamItem & { courseCode: string }> = [];
    for (const c of selectedSemester?.courses ?? []) {
  ...
  const handleAddCourse = React.useCallback(() => {
    ...
    setSemesters((prev) =>
      prev.map((s) => (s.id === selectedSemesterId ? { ...s, courses: [...s.courses, newCourse] } : s))
    );
    setAddNumber('');
    setAddTitle('');
    setAddProfessor('');
  ...
  const handleAddSemester = React.useCallback(() => {
    ...
    if (!name) {
      setToast({ type: 'warning', text: 'Semester name required.' });
      return;
    }
    if (semesters.some((s) => s.id === name)) {
      setToast({ type: 'warning', text: 'Semester already exists.' });
      return;
    }
  ...
  const createStudyGroup = React.useCallback(() => {
    ...
    const newGroup: StudyGroup = {
      ...
      location: sgLocation || (sgIsVirtual ? 'Online' : ''),
